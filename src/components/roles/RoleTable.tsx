...
export function RoleTable({
  ...
  const columns = useMemo<ColumnDef<Role>[]>(
    () => [
      { accessorKey: "name", header: t.roleName, enableSorting: true },
      { accessorKey: "type", header: t.roleType, enableSorting: false },
      { id: "scope", header: t.roleIsGlobal, enableSorting: false },
      { accessorKey: "status", header: t.roleStatus, enableSorting: false },
      { accessorKey: "permissionCount", header: t.rolePermissionCount, enableSorting: false },
  ...
  if (roles.length === 0)
    return (
      <TableEmptyState
        isSearch={!!search}
        icon={<ShieldCheck className="h-8 w-8 text-muted-foreground/40" />}
        title={search ? t.roleNoResults : t.roleEmptyTitle}
        description={t.roleEmptyDesc}
        addLabel={t.roleAdd}
        onAdd={onAdd}
      />
    );
  ...
  const colVisibility: Record<string, string> = {
    ...
    status: "",
    permissionCount: "hidden lg:table-cell",
  };
