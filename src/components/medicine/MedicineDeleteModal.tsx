...
export interface MedicineDeleteModalProps {
  medicine: Medicine;
  onClose: () => void;
  onSuccess: () => void;
}
...
export function MedicineDeleteModal({
  ...
  onClose,
  onSuccess,
}: MedicineDeleteModalProps): JSX.Element {
  const { t } = useLanguage();

  const { mutate, isPending } = useDeleteMutation({
    mutationFn: () => deleteMedicine(medicine.uuid),
    queryKey: ["medicines"],
    onSuccess,
  });
      ...
      >
        <DialogHeader className="flex flex-row items-center gap-3 space-y-0 border-b border-border px-6 py-4">
          ...
          <DialogTitle className="text-base">
            {t.medicineDeleteConfirmTitle}
          </DialogTitle>
        </DialogHeader>

        <div className="px-6 py-5">
          <DialogDescription className="text-sm text-muted-foreground">
            {t.medicineDeleteConfirmDesc}
          </DialogDescription>
