...
import { rejectStockReturn } from "@/service/stockReturnService";

interface FormValues {
  description: string;
}

export interface StockReturnRejectModalProps {
  stockReturn: StockReturn;
  onClose: () => void;
  onSuccess: (updated: StockReturn) => void;
}

export function StockReturnRejectModal({
  stockReturn,
  onClose,
  onSuccess,
}: StockReturnRejectModalProps): JSX.Element {
  ...
  const schema = z.object({
    description: z.string().trim().min(1, t.srRejectionReasonRequired),
  });
  ...
  const mutation = useMutation({
    mutationFn: (values: FormValues) =>
      rejectStockReturn(stockReturn.uuid, { description: values.description }),
    onSuccess: (res) => {
      if (res.success && res.data) {
        queryClient.invalidateQueries({ queryKey: ["stock-returns"] });
        toast.success(t.srRejectSuccess);
    ...
    >
      ...
      >
        <DialogHeader className="flex flex-row items-center gap-3 space-y-0 border-b border-border px-6 py-4">
          ...
          <DialogTitle className="text-base">{t.srRejectConfirmTitle}</DialogTitle>
        </DialogHeader>

        <form onSubmit={handleSubmit((v) => mutation.mutate(v))}>
          <div className="space-y-4 px-6 py-5">
            <DialogDescription className="text-sm text-muted-foreground">
              {t.srRejectConfirmDesc}
            </DialogDescription>
            ...
            <div className="space-y-1.5">
              <Label htmlFor="sr-reject-reason">{t.srRejectionReason}</Label>
              <Textarea
                id="sr-reject-reason"
                {...register("description")}
                placeholder={t.srRejectionReasonPlaceholder}
                rows={3}
                disabled={isPending}
