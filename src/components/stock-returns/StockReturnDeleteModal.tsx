...
import { deleteStockReturn } from "@/service/stockReturnService";

export interface StockReturnDeleteModalProps {
  stockReturn: StockReturn;
  onClose: () => void;
  onSuccess: () => void;
}

export function StockReturnDeleteModal({
  stockReturn,
  onClose,
  onSuccess,
}: StockReturnDeleteModalProps): JSX.Element {
  ...
  const mutation = useMutation({
    mutationFn: () => deleteStockReturn(stockReturn.uuid),
    onSuccess: (res) => {
      if (res.success) {
        queryClient.invalidateQueries({ queryKey: ["stock-returns"] });
        toast.success(t.srDeleteSuccess);
    ...
    >
      ...
      >
        <DialogHeader className="flex flex-row items-center gap-3 space-y-0 border-b border-border px-6 py-4">
          ...
          <DialogTitle className="text-base">{t.srDeleteConfirmTitle}</DialogTitle>
        </DialogHeader>

        <div className="px-6 py-5">
          <DialogDescription className="text-sm text-muted-foreground">
            {t.srDeleteConfirmDesc}
          </DialogDescription>
