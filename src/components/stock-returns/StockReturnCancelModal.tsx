...
import { cancelStockReturn } from "@/service/stockReturnService";

interface FormValues {
  description: string;
}

export interface StockReturnCancelModalProps {
  stockReturn: StockReturn;
  onClose: () => void;
  onSuccess: (updated: StockReturn) => void;
}

export function StockReturnCancelModal({
  stockReturn,
  onClose,
  onSuccess,
}: StockReturnCancelModalProps): JSX.Element {
  ...
  const schema = z.object({
    description: z.string().trim().min(1, t.srCancellationReasonRequired),
  });
  ...
  const mutation = useMutation({
    mutationFn: (values: FormValues) =>
      cancelStockReturn(stockReturn.uuid, { description: values.description }),
    onSuccess: (res) => {
      if (res.success && res.data) {
        queryClient.invalidateQueries({ queryKey: ["stock-returns"] });
        toast.success(t.srCancelSuccess);
    ...
    >
      ...
      >
        <DialogHeader className="flex flex-row items-center gap-3 space-y-0 border-b border-border px-6 py-4">
          ...
          <DialogTitle className="text-base">{t.srCancelConfirmTitle}</DialogTitle>
        </DialogHeader>

        <form onSubmit={handleSubmit((v) => mutation.mutate(v))}>
          <div className="space-y-4 px-6 py-5">
            <DialogDescription className="text-sm text-muted-foreground">
              {t.srCancelConfirmDesc}
            </DialogDescription>
            ...
            <div className="space-y-1.5">
              <Label htmlFor="sr-cancel-reason">{t.srCancellationReason}</Label>
              <Textarea
                id="sr-cancel-reason"
                {...register("description")}
                placeholder={t.srCancellationReasonPlaceholder}
                rows={3}
                disabled={isPending}
