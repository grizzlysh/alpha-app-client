...
export interface PurchaseOrderTableProps {
  ...
  onAdd: () => void;
  onDetails: (o: PurchaseOrder) => void;
  onEdit: (o: PurchaseOrder) => void;
  onPrint: (o: PurchaseOrder) => void;
  onMarkReceived: (o: PurchaseOrder) => void;
  onRepurchase: (o: PurchaseOrder) => void;
...
export function PurchaseOrderTable({
  ...
  onCancel,
  onDelete,
}: PurchaseOrderTableProps): JSX.Element {
  const [sorting, setSorting] = useState<SortingState>([]);

  const columns = useMemo<ColumnDef<PurchaseOrder>[]>(
    () => [
      { accessorKey: "orderNumber", header: t.poOrderNumber, enableSorting: true },
      { accessorKey: "status", header: t.poStatus, enableSorting: false },
      { id: "itemCount", header: t.poItemsSection, enableSorting: false },
      { accessorKey: "orderedAt", header: t.poOrderedAt, enableSorting: true },
      { id: "signedBy", header: t.poSignedBy, enableSorting: false },
  ...
  if (orders.length === 0)
    return (
      <TableEmptyState
        ...
        title={search ? t.poNoResults : t.poEmptyTitle}
        description={t.poEmptyDesc}
        addLabel={t.poAdd}
        onAdd={onAdd}
      />
    );
