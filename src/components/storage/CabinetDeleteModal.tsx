...
import { deleteCabinet } from "@/service/storageService";

export interface CabinetDeleteModalProps {
  cabinet: StorageCabinet;
  onClose: () => void;
  onSuccess: () => void;
}

export function CabinetDeleteModal({
  cabinet,
  onClose,
  onSuccess,
}: CabinetDeleteModalProps): JSX.Element {
  const { t } = useLanguage();

  const { mutate, isPending } = useDeleteMutation({
    mutationFn: () => deleteCabinet(cabinet.uuid),
    queryKey: ["storage-cabinets"],
    onSuccess,
  });
    ...
    <Dialog open onOpenChange={(open) => { if (!open && !isPending) onClose(); }}>
      ...
      >
        <DialogHeader className="flex flex-row items-center gap-3 space-y-0 border-b border-border px-6 py-4">
          ...
          <DialogTitle className="text-base">{t.cabinetDeleteConfirmTitle}</DialogTitle>
        </DialogHeader>

        <div className="px-6 py-5">
          <DialogDescription className="text-sm text-muted-foreground">
            {t.cabinetDeleteConfirmDesc}
          </DialogDescription>
        ...
        <div className="flex justify-end gap-2 border-t border-border px-6 py-4">
          ...
          <Button type="button" variant="destructive" disabled={isPending} onClick={mutate} className="min-w-[7rem] rounded-xl">
            {isPending ? (
              <><Loader2 className="h-4 w-4 animate-spin" />{t.cabinetDeleting}</>
            ) : (
              t.deleteConfirm
            )}
          </Button>
