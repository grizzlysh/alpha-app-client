...
import { deleteBin } from "@/service/storageService";

export interface BinDeleteModalProps {
  bin: StorageBin;
  cabinetUuid: string;
  shelfUuid: string;
  onClose: () => void;
...
export function BinDeleteModal({
  bin,
  cabinetUuid,
  shelfUuid,
  onClose,
  onSuccess,
}: BinDeleteModalProps): JSX.Element {
  const { t } = useLanguage();

  const { mutate, isPending } = useDeleteMutation({
    mutationFn: () => deleteBin(cabinetUuid, shelfUuid, bin.uuid),
    queryKey: ["storage-bins", shelfUuid],
    onSuccess,
  });
    ...
    <Dialog open onOpenChange={(open) => { if (!open && !isPending) onClose(); }}>
      ...
      >
        <DialogHeader className="flex flex-row items-center gap-3 space-y-0 border-b border-border px-6 py-4">
          ...
          <DialogTitle className="text-base">{t.binDeleteConfirmTitle}</DialogTitle>
        </DialogHeader>

        <div className="px-6 py-5">
          <DialogDescription className="text-sm text-muted-foreground">
            {t.binDeleteConfirmDesc}
          </DialogDescription>
        ...
        <div className="flex justify-end gap-2 border-t border-border px-6 py-4">
          ...
          <Button type="button" variant="destructive" disabled={isPending} onClick={mutate} className="min-w-[7rem] rounded-xl">
            {isPending ? (
              <><Loader2 className="h-4 w-4 animate-spin" />{t.binDeleting}</>
            ) : (
              t.deleteConfirm
            )}
          </Button>
