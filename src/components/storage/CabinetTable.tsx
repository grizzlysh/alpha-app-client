...
export interface CabinetTableProps {
  ...
  onAdd: () => void;
  onEdit: (c: StorageCabinet) => void;
  onDelete: (c: StorageCabinet) => void;
  onViewShelves: (c: StorageCabinet) => void;
}
...
export function CabinetTable({
  ...
  onDelete,
  onViewShelves,
}: CabinetTableProps): JSX.Element {
  const [sorting, setSorting] = useState<SortingState>([]);

  const columns = useMemo<ColumnDef<StorageCabinet>[]>(
    () => [
      { accessorKey: "name", header: t.cabinetName, enableSorting: true },
      { accessorKey: "description", header: t.cabinetDescription, enableSorting: false },
      { accessorKey: "status", header: t.cabinetStatus, enableSorting: true },
      { id: "shelves", header: "", enableSorting: false },
  ...
  if (cabinets.length === 0)
    return (
      <TableEmptyState
        isSearch={!!search}
        icon={<Warehouse className="h-8 w-8 text-muted-foreground/40" />}
        title={search ? t.cabinetNoResults : t.cabinetEmptyTitle}
        description={t.cabinetEmptyDesc}
        addLabel={t.cabinetAdd}
        onAdd={onAdd}
      />
    );
          ...
          <tbody className="divide-y divide-border">
            {rows.map((row) => (
              <CabinetRow
                ...
                cabinet={row.original}
                t={t}
                onEdit={() => onEdit(row.original)}
                onDelete={() => onDelete(row.original)}
                onViewShelves={() => onViewShelves(row.original)}
              />
            ))}
          </tbody>
        </table>
