...
import type { StorageCabinet, CreateCabinetPayload, UpdateCabinetPayload } from "@/types/storage";
import { createCabinet, updateCabinet } from "@/service/storageService";

// ── Schema ────────────────────────────────────────────────────────────────────

function makeSchema(t: Translations) {
  return z.object({
    name: z.string().min(1, t.cabinetNameRequired),
    code: z.string().min(1, t.cabinetCodeRequired),
...
export function CabinetFormModal({
  ...
  onClose,
  onSuccess,
}: CabinetFormModalProps): JSX.Element {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (res) => {
      if (res.success) {
        queryClient.invalidateQueries({ queryKey: ["storage-cabinets"] });
        toast.success(<LiveToastMessage getMessage={(t) => t.cabinetCreateSuccess} />);
  ...
  const updateMutation = useMutation({
    mutationFn: ({ uuid, payload }: { uuid: string; payload: UpdateCabinetPayload }) =>
      updateCabinet(uuid, payload),
    onSuccess: (res) => {
      if (res.success) {
        queryClient.invalidateQueries({ queryKey: ["storage-cabinets"] });
        toast.success(<LiveToastMessage getMessage={(t) => t.cabinetUpdateSuccess} />);
        onSuccessRef.current();
      } else {
        toast.error(res.message[language]);
      }
  ...
  function onSubmit(values: FormValues): void {
    if (mode === "create") {
      ...
      const payload: UpdateCabinetPayload = {
        name: values.name,
        code: values.code,
        description: values.description || undefined,
        status: values.status,
      };
      updateMutation.mutate({ uuid: cabinet.uuid, payload });
    }
  }
      ...
      >
        <DialogHeader className="flex flex-row items-center gap-3 space-y-0 border-b border-border px-6 py-4">
          ...
          <DialogTitle className="text-base">
            {mode === "create" ? t.cabinetAdd : t.cabinetEdit}
          </DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={form.handleSubmit(onSubmit as SubmitHandler<FormValues>)}>
          <div className="px-6 py-5">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              {/* Code */}
              <div className="space-y-1.5 sm:col-span-2">
                <Label htmlFor="cab-code">
                  {t.cabinetCode}
                  <span className="ml-0.5 text-destructive">*</span>
                </Label>
                <Input
                  id="cab-code"
                  placeholder={t.cabinetCodePlaceholder}
                  {...form.register("code", {
                    onChange: (e) => {
              ...
              {/* Description */}
              <div className="space-y-1.5 sm:col-span-2">
                <Label htmlFor="cab-desc">{t.cabinetDescription}</Label>
                <textarea
                  id="cab-desc"
                  rows={2}
                  placeholder={t.cabinetDescriptionPlaceholder}
          ...
          <div className="flex justify-end gap-2 border-t border-border px-6 py-4">
            ...
            <Button type="submit" disabled={isPending} className="min-w-[7rem] rounded-xl">
              {isPending ? (
                <><Loader2 className="h-4 w-4 animate-spin" />{t.cabinetSaving}</>
              ) : (
                t.cabinetSave
              )}
            </Button>
