...
import { CHART_TOOLTIP_STYLE } from "./dashboardUtils";

export interface DashboardTopByQuantityCardProps {
  t: Translations;
  data: TopProductItem[];
  isLoading: boolean;
}

export function DashboardTopByQuantityCard({
  t,
  data,
  isLoading,
}: DashboardTopByQuantityCardProps): JSX.Element {
  return (
    <Card className="p-4">
      <div className="flex items-center gap-2 text-sm font-semibold text-foreground">
        <Hash className="h-4 w-4 text-muted-foreground" />
        {t.dashboardTopByQtyTitle}
      </div>
