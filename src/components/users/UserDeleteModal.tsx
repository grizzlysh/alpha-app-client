...
export function UserDeleteModal({
  ...
  const { mutate, isPending } = useDeleteMutation({
    mutationFn: () => deleteUser(user.uuid),
    queryKey: ["users"],
    onSuccess,
  });
    ...
    <Dialog open onOpenChange={(open) => { if (!open && !isPending) onClose(); }}>
      ...
      >
        <DialogHeader className="flex flex-row items-center gap-3 space-y-0 border-b border-border px-6 py-4">
          ...
          <DialogTitle className="text-base">{t.userDeleteConfirmTitle}</DialogTitle>
        </DialogHeader>
        ...
        <div className="flex justify-end gap-2 border-t border-border px-6 py-4">
          ...
          >
            {isPending ? (
              <><Loader2 className="h-4 w-4 animate-spin" /> {t.userDeleting}</>
            ) : t.deleteConfirm}
          </Button>
        </div>
      </DialogContent>
