...
import { cancelSale } from "@/service/saleService";

interface FormValues {
  description: string;
}

export interface SaleCancelModalProps {
  sale: Sale;
  onClose: () => void;
...
export function SaleCancelModal({
  ...
  onClose,
  onSuccess,
}: SaleCancelModalProps): JSX.Element {
  ...
  const schema = z.object({
    description: z.string().trim().min(1, t.saleCancelReasonRequired),
  });
  ...
  const mutation = useMutation({
    mutationFn: (values: FormValues) =>
      cancelSale(sale.uuid, { description: values.description }),
    onSuccess: (res) => {
      if (res.success && res.data) {
        queryClient.invalidateQueries({ queryKey: ["sales"] });
        queryClient.invalidateQueries({ queryKey: ["sale", sale.uuid] });
        toast.success(<LiveToastMessage getMessage={(t) => t.saleCancelSuccess} />);
    ...
    >
      ...
      >
        <DialogHeader className="flex flex-row items-center gap-3 space-y-0 border-b border-border px-6 py-4">
          ...
          <DialogTitle className="text-base">{t.saleCancelConfirmTitle}</DialogTitle>
        </DialogHeader>

        <form onSubmit={handleSubmit((v) => mutation.mutate(v))}>
          <div className="space-y-4 px-6 py-5">
            <DialogDescription className="text-sm text-muted-foreground">
              {t.saleCancelConfirmDesc}
            </DialogDescription>
            ...
            <div className="space-y-1.5">
              <Label htmlFor="sale-cancel-reason">{t.saleCancelReason}</Label>
              <Textarea
                id="sale-cancel-reason"
                {...register("description")}
                placeholder={t.saleCancelReasonPlaceholder}
                rows={3}
                disabled={isPending}
