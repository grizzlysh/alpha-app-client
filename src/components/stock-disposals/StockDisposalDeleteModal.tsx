...
import { deleteStockDisposal } from "@/service/stockDisposalService";

export interface StockDisposalDeleteModalProps {
  disposal: StockDisposal;
  onClose: () => void;
  onSuccess: () => void;
}

export function StockDisposalDeleteModal({
  disposal,
  onClose,
  onSuccess,
}: StockDisposalDeleteModalProps): JSX.Element {
  ...
  const mutation = useMutation({
    mutationFn: () => deleteStockDisposal(disposal.uuid),
    onSuccess: (res) => {
      if (res.success) {
        queryClient.invalidateQueries({ queryKey: ["stock-disposals"] });
        toast.success(t.sdDeleteSuccess);
    ...
    >
      ...
      >
        <DialogHeader className="flex flex-row items-center gap-3 space-y-0 border-b border-border px-6 py-4">
          ...
          <DialogTitle className="text-base">{t.sdDeleteConfirmTitle}</DialogTitle>
        </DialogHeader>

        <div className="px-6 py-5">
          <DialogDescription className="text-sm text-muted-foreground">
            {t.sdDeleteConfirmDesc}
          </DialogDescription>
